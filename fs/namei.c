...
int sysctl_prot_symlinks_create __read_mostly = 0;
int sysctl_prot_hardlinks_create __read_mostly = 0;
int sysctl_prot_symlinks_allow_gid __read_mostly = 0;
int sysctl_prot_hardlinks_allow_gid __read_mostly = 0;
...
static int check_link_group_permission(const struct inode *inode, bool symlink)
{
	int error = -EACCES;
	gid_t target_gid;
	...
	if (symlink == true) {
		if (sysctl_prot_symlinks_allow_gid) {
			if (target_gid == sysctl_prot_symlinks_allow_gid)
				error = 0;
		}
	} else {
		if (sysctl_prot_hardlinks_allow_gid) {
			if (target_gid == sysctl_prot_hardlinks_allow_gid)
				error = 0;
		}
	}
...
static int may_create_hard_link(struct path *path)
{
	int error = 0;
	struct inode *inode;
	...
	if (uid_eq(current_fsuid(), inode->i_uid) != true)
...
static int may_create_sym_link(const char *name, struct path *path,
		struct inode *inode)
{
	int err = 0;
	struct filename *filename;
	...
	current->total_link_count = 0;
	err = link_path_walk(filename->name, &nd);

	if (!err && !(nd.flags & LOOKUP_PARENT)) {
		err = lookup_last(&nd, &_path);
		while (err > 0) {
			void *cookie;
			struct path link = _path;
			err = may_follow_link(&link, &nd);
			if (unlikely(err))
				break;
			nd.flags |= LOOKUP_PARENT;
	...
	if (uid_eq(current_fsuid(), inode->i_uid) != true)
		err = check_link_group_permission(inode, true);
		...
		int, newdfd, const char __user *, newname)
{
	...
	if (sysctl_prot_symlinks_create != 0) {
		struct filename *to;
		struct nameidata nd;
		...
		error = may_create_sym_link(oldname, &nd.path, nd.inode);
		putname(to);
		path_put(&nd.path);
		if (error != 0)
			goto out_putname;
...
retry:
	...
	if (sysctl_prot_hardlinks_create != 0) {
		error = may_create_hard_link(&old_path);
		if (error)
			goto out;
	}
