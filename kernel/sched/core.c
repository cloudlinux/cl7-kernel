...
static void (*loadavg_ptr)(void);
static unsigned long loadavg_ptr_bits;
static DECLARE_WAIT_QUEUE_HEAD(loadavg_ptr_wq);
#define LA_PTR_STOP_BIT        0

void loadavg_ptr_fini()
{
	if (loadavg_ptr != NULL) {
		set_bit(LA_PTR_STOP_BIT, &loadavg_ptr_bits);
		wait_event(loadavg_ptr_wq, (loadavg_ptr == NULL));
	}
}
EXPORT_SYMBOL(loadavg_ptr_init);

void loadavg_ptr_init(void (*ptr)(void))
{
	loadavg_ptr = ptr;
}
EXPORT_SYMBOL(loadavg_ptr_fini);
...
void calc_global_load(unsigned long ticks)
{
	...
	if (loadavg_ptr != NULL) {
		(*loadavg_ptr)();
		if (test_bit(LA_PTR_STOP_BIT, &loadavg_ptr_bits)) {
			loadavg_ptr = NULL;
			wake_up(&loadavg_ptr_wq);
			clear_bit(LA_PTR_STOP_BIT, &loadavg_ptr_bits);
		}
